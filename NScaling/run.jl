...
const extract_diagonal = false
const proj_weights = [1, 1, -1, -1]

const n_resamples = parse(Int, ARGS[2])

const N_Max = parse(Int, ARGS[3])
const n_N = Int(log2(N_Max))
const N_arr = [2 ^ i for i in 1:n_N]

const ϵ_angles = parse(Float64, ARGS[4]) * π

const ϵ_arr = [0.06905, 0.054836, 0.05215, 0.05152]

println(ARGS[5])
@assert ARGS[5][1] == '[' && ARGS[5][end] == ']'
@assert ARGS[7][1] == '[' && ARGS[6][end] == ']'
@assert ARGS[6][1] == '[' && ARGS[7][end] == ']'

const n_samples_populations_arr = parse.(Int, split(chop(ARGS[5], head=1),','))
    # Number of population samples for the initial state for each N
const n_samples_phase_estimation_each_arr = parse.(Int, split(chop(ARGS[6], head=1),','))
    # Number of phase estimation samples for each DTQW for each N
const n_samples_compound_each_arr = parse.(Int, split(chop(ARGS[7], head=1),','))
    # Number of samples for each of the N-1 compound-setting DTQWs for each N

Random.seed!(8675309) # / Jenny dont change your number

n_samples_total = zeros(Int64, n_N)
...
for (idx_N, N) in enumerate(N_arr)
    # iterate over all time-bin numbers N

    Ψ_mes = insert_initial_state(correlated_timebin_state(fill(1 / sqrt(N), N)))
        # prepare the maximally-entangled reference state Ψ_MES

    angles_single = angles_single_setup(N)
        # compute the angles for the single-setting DTQW scheme

    ϵ = ϵ_arr[idx_N]
        # dephasing parameter for the initial state
    ...
    n_samples_phase_estimation_each = n_samples_phase_estimation_each_arr[idx_N]
    n_samples_populations = n_samples_populations_arr[idx_N]
    n_samples_compound_each = n_samples_compound_each_arr[idx_N]
        # number of samples for the phase estimation, initial-state populations and compound-setting DTQW scheme

    n_samples_single_setting = (N - 1) * n_samples_compound_each
        # number of samples for the single-setting DTQW scheme

    n_samples_total[idx_N] =
        n_samples_populations + 2 * n_samples_phase_estimation_each + n_samples_single_setting
        # total number of samples used in each experimental run
    ...
    tasks = map(data_chunks) do package
        ...
        Base.Threads.@spawn begin
            ...
            for i in 1:chunk_size
                ...
                ) # calculate the sampled noisy final-state populations of the compound-setting DTQWs for the dephased/pure initial states
   ...
   # collect all results
    ...
    data_path = string(@__DIR__,"/Data/N$N.jld2")
    save(data_path,
    ...
    F_avg_single = mean(fidelity_sampled_single_deph)
    F_std_single = std(fidelity_sampled_single_deph)
    F_avg_compound = mean(fidelity_sampled_compound_deph)
    F_std_compound = std(fidelity_sampled_compound_deph)
        # compute the mean and standard deviation of the fidelities of single/compound-setting schemes for the dephased initial state

    F_avg_single_pure = mean(fidelity_sampled_single_pure)
    F_std_single_pure = std(fidelity_sampled_single_pure)
    F_avg_compound_pure = mean(fidelity_sampled_compound_pure)
    F_std_compound_pure = std(fidelity_sampled_compound_pure)
        # compute the mean and standard deviation of the fidelities of single/compound-setting schemes for the pure initial state


    F_avg_true = mean(mes_fidelity_deph)
    F_std_true = std(mes_fidelity_deph)
        # compute the mean and standard deviation of the true fidelities of the dephased initial state after phase correction

    F_avg_true_pure = mean(mes_fidelity_pure)
    ...
    println("")
    println("")
    println("N = $N | samples used: $(n_samples_total[idx_N])")
    println("")
    println("Purity tr(ρ^2) = 1")
    println("True Fidelity ρ_corr to Ψ_MES = $(mes_fidelity_pure[1]) ± $F_std_true_pure | F_avg = $F_avg_true_pure")
    println("Single Setup: F = $(fidelity_sampled_single_pure[1]) ± $F_std_single_pure | F_avg = $F_avg_single_pure")
    println("Compound Setup: F = $(fidelity_sampled_compound_pure[1]) ± $F_std_compound_pure | F_avg = $F_avg_compound_pure")
    println("")
    println("Purity tr(ρ^2) ≈ 0.9")
    println("Max. Fidelity perfect phase correction = $(fidelity(Ψ_mes, ρ_perfect_phase_correction))")
    println("True Fidelity ρ_corr to Ψ_MES = $(mes_fidelity_deph[1]) ± $F_std_true | F_avg = $F_avg_true")
    println("Single Setup: F = $(fidelity_sampled_single_deph[1]) ± $F_std_single | F_avg = $F_avg_single")
    println("Compound Setup: F = $(fidelity_sampled_compound_deph[1]) ± $F_std_compound | F_avg = $F_avg_compound")
